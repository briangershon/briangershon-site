...
class SEOLinter {
  ...
  checkTitle(document, filePath) {
    ...
    if (this.duplicates.has(title)) {
      this.log(
        "warning",
        filePath,
        `Duplicate title: "${title}" (also in ${this.duplicates.get(title)})`
      );
    } else {
  ...
  checkMetaDescription(document, filePath) {
    ...
    if (description.length < 120) {
      this.log(
        "warning",
        filePath,
        `Description too short (${description.length} chars, recommended 120-160)`
      );
    } else if (description.length > 160) {
      this.log(
        "warning",
        filePath,
        `Description too long (${description.length} chars, recommended 120-160)`
      );
    }
  }
  ...
  checkArticleMetadata(document, filePath) {
    ...
    if (isArticle) {
      ...
      // const articleTagElements = document.querySelectorAll(
    ...
    for (let i = 0; i < jsonLdScripts.length; i++) {
      ...
      try {
        ...
        this.log(
          "error",
          filePath,
          `Invalid JSON-LD syntax in script ${i + 1}: ${error.message}`
        );
      }
    }
  }
  ...
  validateJsonLD(jsonLD, filePath, scriptNum) {
    ...
    if (!jsonLD["@context"]) {
      this.log(
        "error",
        filePath,
        `JSON-LD script ${scriptNum}: Missing @context`
      );
    }
    ...
    if (jsonLD.author && typeof jsonLD.author === "object") {
      if (!jsonLD.author["@type"] || jsonLD.author["@type"] !== "Person") {
        this.log(
          "warning",
          filePath,
          `JSON-LD script ${scriptNum}: Author should be of type "Person"`
        );
      }
      if (!jsonLD.author.name) {
        this.log(
          "error",
          filePath,
          `JSON-LD script ${scriptNum}: Author missing name`
        );
      }
    }
  }
  ...
  checkRSSFeed(document, filePath) {
    const rssFeed = document.querySelector(
  ...
  convertToSourcePath(htmlPath) {
    // Convert _site/blog/{slug}/index.html to src/blog/posts/{slug}.md
    const blogMatch = htmlPath.match(/_site\/blog\/([^/]+)\/index\.html$/);
    if (blogMatch) {
      return `src/blog/posts/${blogMatch[1]}.md`;
    }

    // Convert _site/index.html to src/index.liquid
    if (htmlPath === "_site/index.html") {
      return "src/index.liquid";
    }

    // For other paths, try to map back to src
    const sitePath = htmlPath.replace(/^_site\//, "src/");
    return sitePath.replace(/\/index\.html$/, ".liquid");
  }
  ...
  async run() {
    ...
    try {
      ...
      if (htmlFiles.length === 0) {
        console.log(
          "❌ No HTML files found in _site directory. Run `npm run build` first."
        );
        return 1;
      }
  ...
  printResults() {
    ...
    for (const [file, issues] of fileGroups) {
      const sourcePath = this.convertToSourcePath(file);
